...
import ru.geekbrains_studies.IOBI.Menus.MainMenu;

import java.util.Date;
import java.util.List;
import java.util.Scanner;

public class ConsoleIOBI {
    public static final int SIZE_LINE = 150;
    ...
    public void start() {
        ...
        printCaption("СПИСОК ЖИВОТНЫХ", Colors.GREEN);
        printLineWithSymbol("=", SIZE_LINE, Colors.YELLOW);
        m.showAnimals();
        printLineWithSymbol("-", SIZE_LINE, Colors.YELLOW);
        while (isWork) {
    ...
    private void printCaption(String caption, Enum displayColor) {
        int spaceSize = (SIZE_LINE - caption.length()) / 2;
        String captionLine = " ".repeat(spaceSize) + caption + " ".repeat(spaceSize);
